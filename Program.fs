...
module PascalCaseSingleArgumentBracketsRule =
    ...
    let find_missing_brackets_oak (oak: Oak) : ResizeArray<pos * pos> =
        ...
        let rec walk_all_applications (n: Node) =
            ...
            | _ -> ()
            for c in n.Children do walk_all_applications(c)
        for c in oak.Children do walk_all_applications(c)
...
let main argv : int =
    ...
    let files =
        ...
        |> Array.filter(fun path ->
            let relative = Path.GetRelativePath(Directory.GetCurrentDirectory(), path).Replace("\\", "/")
